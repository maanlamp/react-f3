...
export const errorChain = <Schema extends ZodObject>(
	...
	new Proxy(
		{},
		{ get: (_, k) => getErrorByPath(issues, [k]) }
	) as any;
...
export interface ArrayErrorGetter<T>
	extends ErrorGetter {
	(index: number): T;
}
...
export type ErrorChainFromSchema<T extends ZodObject> =
	ErrorChain<DeepNonNullable<z.core.infer<T>>>;
